...
for (const value of languages) {
  ...
  // for문 : 모든 데이터 순환용 => 작성 코드가 많다. => 무한루프의 여지가 생길 수 있음
  // for ... in 문 : 객체 순환용 -> 정해진 아이템의 갯수만큼 반복 => 배열 순환용으로는 좋지 않음
  // for ... of 문 : 배열순환용 (iterable 요소 순환용) => 정해진 아이템의 갯수만큼 반복 => 배열 순환용으로 적합
  ...
  const keys = Object.keys(obj); // 객체의 key들을 모아 새로운 배열을 반환하는 유틸 함수
  const values = Object.values(obj); // 객체의 value들을 모아 새로운 배열을 반환하는 유틸 함수
  const entries = Object.entries(obj); //객체의 [key, value]들을 모아 한 쌍의 배열을 이루는 새로운 배열을 반환하는 유틸 함수
  ...
  const randomUser = {
    ...
    login: {
      ...
      sha256:
        '9e49256b8917113750533c24c015336af43d5d7130cf8faa19054c1ba36e7de8',
    },
